...
func TestErrorsInNativeCode(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "Uncaught exception: TypeError: undefined is not a function") {
		t.Error("Expected error did not contain message: ", err)
	}
	if !strings.Contains(err.Error(), "at map_undef.js:1:3") {
		t.Error("Expected error did not contain location: ", err)
	}
	if !strings.Contains(err.Error(), "Stack trace: TypeError: undefined is not a function") {
		t.Error("Expected error did not contain stack trace: ", err)
	}
}
