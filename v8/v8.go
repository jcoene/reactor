...
func NewIsolateWithSnapshot(s *Snapshot) *Isolate {
	...
	runtime.SetFinalizer(iso, func(i *Isolate) {
		println("newIsolateWithSnapshot finalizer")
		i.release()
	})
	return iso
}
...
func (ctx *Context) newValue(ptr C.PersistentValuePtr) *Value {
	...
	runtime.SetFinalizer(val, func(v *Value) {
		println("ctx.newValue finalizer")
		v.release()
	})
	return val
}
